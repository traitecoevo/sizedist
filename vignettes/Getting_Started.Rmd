...
m_g_fitted <- extract_fitted(m_g_fit, pars_cm_kg)

plot_size_dist(data, pars, binwidth = 0.1, m_g_fitted)
```

#### 1d: Estimating Growth, Mortality and size-at-hatch.

There is a version of model1 (`model1d`) which also estimates size-at-hatch `s0` from the size-at-age and size data using a similiar method to `model1c`. 
...
```{r}
m_g_s0_fit <- fit_sizedist(standata_size_growth_mod4)

print(m_g_s0_fit, probs=c(0.025, 0.5, 0.975))
```
...
m_g_s0_fitted <- extract_fitted(m_g_s0_fit, pars_cm_kg)

plot_size_dist(data, pars, binwidth = 0.1, m_g_s0_fitted)
```

#### 1e: Estimating Slope of abundance spectra.

The final version of model1 (`model1e`) fits an abundance spectra model (change in abundance as a function of size) assuming constant growth.
...
# Add in parameters to data list
standata_size_mod <- standata_size %>% add_pars(pars_abund)
...
```{r}
abund_fit <- fit_sizedist(standata_size_mod)

print(abund_fit, probs=c(0.025, 0.5, 0.975))
```
...
```{r}
model2a_fit <- fit_sizedist(standata_size_mod)
...
The `model2b` variant estimates the size-at-hatch $s_0$, growth rate $g ̅$ and mortality rate $z ̅$
...
for(i in seq_len(nrow(dat2))) {
  p <- p + geom_abline(intercept = s0, slope = dat2$g[i], col = "blue", alpha = 0.1) #fit posterior draws
  }
