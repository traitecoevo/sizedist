...
```{r}
group_pars <- 
  tibble(level3_index = as.character(1:4), 
         g_gl = seq(0.15, 0.25, length.out=4)) %>% 
  expand_grid(
         Z_gl = seq(0.2, 0.25, length.out=4)
          ) %>% 
  mutate(level2_index = as.character(seq_len(n()))
            )  %>% 
  expand_grid(
         R_gl = seq(100, 1000, length.out=4)
          ) %>% 
  mutate(
...
sample_pars %>% 
  ggplot(aes(as.character(level3_index), Z_av, col = level2_index, group = level2_index)) +
  geom_boxplot()
