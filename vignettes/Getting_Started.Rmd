...
First select default pars for `model2a` to simulate a test population
...
pars1 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = 100,
                                 g_av = 0.1,
                                 Z_av = 0.2))
...
#Sample1
pars1 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,1000,100)),
                                 g_av = rnorm(1,0.2,0.01),
                                 Z_av = rnorm(1,0.2,0.03)))

data1 <- simulate_population(pars1) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)
...
pars2 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,1000,100)),
                                 g_av = rnorm(1,0.2,0.03),
                                 Z_av = rnorm(1,0.2,0.03)))

data2 <- simulate_population(pars2) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)
...
data3 <- simulate_population(pars3) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)
...
#Sample4
pars4 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,1000,100)),
                                 g_av = rnorm(1,0.2,0.03),
                                 Z_av = rnorm(1,0.2,0.03)))

data4 <- simulate_population(pars4) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data4$sample_index <- 4
data4$group_index <- 1

#Sample5
pars5 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,1200,100)),
                                 g_av = rnorm(1,0.22,0.03),
                                 Z_av = rnorm(1,0.2,0.03)))

data5 <- simulate_population(pars5) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data5$sample_index <- 5
data5$group_index <- 2

#Sample6
pars6 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,1200,100)),
                                 g_av = rnorm(1,0.22,0.03),
                                 Z_av = rnorm(1,0.2,0.03)))

data6 <- simulate_population(pars6) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data6$sample_index <- 6
data6$group_index <- 2

#sample7
pars7 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,1200,100)),
                                 g_av = rnorm(1,0.22,0.03),
                                 Z_av = rnorm(1,0.2,0.03)))

data7 <- simulate_population(pars7) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data7$sample_index <- 7
data7$group_index <- 2


#Sample8
pars8 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,1200,100)),
                                 g_av = rnorm(1,0.22,0.03),
                                 Z_av = rnorm(1,0.2,0.03)))

data8 <- simulate_population(pars8) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data8$sample_index <- 8
data8$group_index <- 2


#Sample9
pars9 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,800,100)),
                                 g_av = rnorm(1,0.20,0.03),
                                 Z_av = rnorm(1,0.22,0.03)))

data9 <- simulate_population(pars9) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data9$sample_index <- 9
data9$group_index <- 3

#Sample10
pars10 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,800,100)),
                                 g_av = rnorm(1,0.20,0.03),
                                 Z_av = rnorm(1,0.22,0.03)))
                     
data10 <- simulate_population(pars10) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data10$sample_index <- 10
data10$group_index <- 3

#Sample11
pars11 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,800,100)),
                                 g_av = rnorm(1,0.20,0.03),
                                 Z_av = rnorm(1,0.22,0.03)))

data11 <- simulate_population(pars11) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data11$sample_index <- 11
data11$group_index <- 3


#Sample12
pars12 <- default_pars("model2") %>%
  purrr::list_modify(pars = list(R = round(rnorm(1,800,100)),
                                 g_av = rnorm(1,0.20,0.03),
                                 Z_av = rnorm(1,0.22,0.03)))

data12 <- simulate_population(pars12) %>% summarise_bin_counts(bin_var = size, 
                     bin_width = 0.1)

data12$sample_index <- 12
data12$group_index <- 3
```
