...
Once the name of the model has been updated, the `pars` object will need to be joined to the data list created above. Note that `add_pars()` will by default, prune the `pars` list to the parameters that are used in model fitting. 
```{r fit}
...
Similar to the first example, we will know need to add these parameters to our data list in order to run our model with `add_pars()`.
...
To fit this final model (`model1c`), we will again need to update our parameters and add them to our combined data list before fitting. Again, you can supply your own value for `s0_av` which represents the average size at hatching/birth using `list_modify()`
