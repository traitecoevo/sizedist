...
nuts_1: mcmc_nuts_energy()
Produces overlaid histograms showing energy__ πE (i.e., the value of the Hamiltonian (up to an additive constant) at each sample) vs the change in energy__ πΔE (See Betancourt (2016) for details.) 
In the figure, we are looking to see if the histograms for πE and πΔE are approximately equal. If the distribution of πΔE has deviated from πE (e.g., wider tails) then autocorrelation of the chains will be large.

Betancourt, M. (2016). Diagnosing suboptimal cotangent disintegrations in Hamiltonian Monte
Carlo. https://arxiv.org/abs/1604.00695


nuts_2, nuts_3, nuts_4: mcmc_scatter()
Produces bivariate scatterplots of the MCMC draws, with the divergent chains highlighted. Here we are checking to see if the divergent chains are clustered in parameter space. If the divergent chains are spread out, then they are most probably false positive results and can be discounted. If the divergences are clustered, it is indicative that sampling has not computed a reliable answer. More details on why the scatter plot with divergences is a useful diagnostic tool can be found in Gabry et al. (2019).
...
How well do the fitted lines match the data?
