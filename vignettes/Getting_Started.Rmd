...
To fit this final model (`model1c`), we will again need to update our hyperparameters and add them to our combined data list before fitting. Again, you can supply your own value for `s0_av` which represents the average size at hatching/birth using `list_modify()`

```{r}
pars_m_g <- list_modify(pars,
                        model = "model1c")
