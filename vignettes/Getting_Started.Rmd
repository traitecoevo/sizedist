...
### Simulating age structured data
...
```{r}
(pars <- pars %>% mutate_pars(model = "model1_a"))
```

Once the name of the model has been updated, the `pars` object will need to be joined to the data list created above. 
```{r fit}
standata_age_mod1 <- standata_age %>% add_pars(pars = pars) 
...
```{r, fig.align='center'}
plot_size_dist(data, default_pars("model1"), binwidth = 0.1)
```
...
standata_growth <- 
  ...
  dplyr::mutate(size= size + rnorm(length(size), 0, 0.1)) %>%
...
Next, we can plot the trace chains to see if they converged or not. Are some chains getting stuck? What you want to see here is all four chains to overlap each other across most of the iterations
...
```{r, message=FALSE, fig.align='center', fig.height=7, fig.width=8}
...
growth_data <- data.frame(age = standata_size_growth_mod3$age,
                          size_ind = standata_size_growth_mod3$size_ind)

s0 = standata_size_growth_mod3$s0_av
