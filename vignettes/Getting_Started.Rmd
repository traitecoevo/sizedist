...
In order to fit the *Constant Mortality, known Growth model* (`model1_b`), we will need to edit the model name using `mutate_pars()`. Similarly,you can edit the values  for `g_av` which represents the average growth rate and `s0_av` which represents the average size at hatching/birth within the same function. 
```{r}
