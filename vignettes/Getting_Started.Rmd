...
group_pars <- 
  tibble(level2_index = as.character(1:3), 
         g_gl = seq(0.15, 0.25, length.out=3),
         Z_gl = 0.2) %>% 
  expand_grid(
         R_gl = seq(500, 500, length.out=3)
          ) %>% 
  mutate(
            level1_index = as.character(seq_len(n()))
            )

generate_sample_pars <- function(R_gl, g_gl, Z_gl) {
  
  tibble(
    R = round(rnorm(1, R_gl,1)),
...
sample_pars <- 
  group_pars %>% 
  mutate(
    data = purrr::pmap(list(R_gl, g_gl, Z_gl), generate_sample_pars)) %>% 
  unnest(data)

sample_pars %>% 
  ggplot(aes(as.character(level2_index), g_av)) +
  geom_boxplot()
...
data <- 
  data_full %>%
  select(level2_index, level1_index, counts) %>% 
  unnest(counts)


sample_pars %>% 
  ggplot(aes(as.character(level2_index), 1+(Z_av/g_av), col = level2_index, group = level2_index)) +
  geom_boxplot()

pars <- sample_pars %>% 
  group_by(level2_index) %>% 
  mutate(level2_mu = mean(1+(Z_av/g_av))) %>% 
  ungroup() %>% 
  select(level2_index, level2_mu) %>% 
  distinct()
...
samples <- 
  data %>% select(level1_index) %>% distinct() %>%
  mutate(level1_index = seq_len(n()))

groups <- data %>% select(level2_index) %>% distinct() %>%
  mutate(level2_index = seq_len(n()))
```

create group index of length N_samples

```{r}
group_by_sample <-data %>% select(level2_index, level1_index) %>% distinct()

#create staion index of length N_tows
group_num <- sort(unique(data$level2_index))

match_group <- match(group_by_sample$level2_index, group_num)
```
...
standata_size <- data %>% 
  compose_count_data() %>%
  purrr::list_modify(N_level1 = nrow(samples),
                     N_level2 = nrow(groups),
                     level2_index = match_group
                     )
```
...
```{r}
posterior <- as.matrix(model2a_h2_fit)
...
par_1 <- sample_pars %>% 
  ...
  geom_point() +
  coord_flip() +
  scale_y_log10(limits = c(0.9, 3.5), breaks = c(1, 2, 3)) +
  scale_x_discrete(limits=rev) +
  xlab("Level 2") +
  ylab("exponent") +
  theme_bw() +
  ggtitle("Exponent used to simulate population")
...
par_2 <- sample_pars %>% 
  ggplot(aes(as.character(level1_index), 1+(Z_av/g_av), col = level1_index, group = level1_index)) +
  geom_boxplot() +
  geom_point() +
  coord_flip() +
  scale_y_log10(limits = c(1.7, 3), breaks = c(1.75, 2.25, 2.75)) +
  scale_x_discrete(limits=rev) +
  xlab("Level 1") +
  ylab("exponent") +
  theme_bw() +
...
```{r}
(post_1 | par_1) / (post_2 | par_2)
```

#### 2a_h3: 3 Random effects

`model2a_h3` contains 3 random effects, estimate c and b for level1, level2, and level3, as well as global estimates.


To begin, create data with three 'levels', where 'level1' is the individual sample, and levels 2 & 3 are hierarchical groups.


```{r}
group_pars <- 
  tibble(level3_index = as.character(1:3), 
         g_gl = seq(0.15, 0.25, length.out=3)) %>% 
  expand_grid(
         Z_gl = seq(0.2, 0.25, length.out=3)
          ) %>% 
  mutate(level2_index = as.character(seq_len(n()))
            )  %>% 
  expand_grid(
         R_gl = seq(300, 300, length.out=3)
          ) %>% 
  mutate(
            level1_index = as.character(seq_len(n()))
            )
...
generate_sample_pars <- function(R_gl, g_gl, Z_gl) {
  ...
  tibble(
    R = round(rnorm(1, R_gl,1)),
    g_av = rnorm(1, g_gl, 0.005),
    Z_av = rnorm(1,Z_gl,0.005))
}  
...
sample_pars %>% 
  ggplot(aes(as.character(level3_index), g_av, col = level2_index, group = level2_index)) +
  geom_boxplot()

sample_pars %>% 
  ggplot(aes(as.character(level3_index), 1+(Z_av/g_av), col = level2_index, group = level2_index)) +
  geom_boxplot()
...
data <- 
  data_full %>%
  select(level3_index, level2_index, level1_index, counts) %>% 
  unnest(counts)
...
level2 <- data %>% select(level2_index) %>% distinct() %>%
  mutate(level2_index = seq_len(n()))

level3 <- data %>% select(level3_index) %>% distinct() %>%
  mutate(level3_index = seq_len(n()))
```

create Level2 index of length N_level1
```{r}
group_by_sample <- data %>% select(level2_index, level1_index) %>% distinct()

#create staion index of length N_tows
group_level2 <- sort(unique(data$level2_index))

match_level2 <- match(group_by_sample$level2_index, group_level2)
```

create Level3 index of length N_level3

```{r}
group_by_sample <- data %>% select(level3_index, level2_index) %>% distinct()

#create staion index of length N_tows
group_level3 <- sort(unique(data$level3_index))

match_level3 <- match(group_by_sample$level3_index, group_level3)
```
...
standata_size <- data %>% 
  compose_count_data() %>%
  purrr::list_modify(N_level1 = nrow(level1),
                     N_level2 = nrow(level2),
                     N_level3 = nrow(level3),
                     level2_index = match_level2,
                     level3_index = match_level3,
                     )
```

Add priors for `model2a_h3`
```{r}
pars  <- default_pars("model2")

pars_model2a_h3 <- purrr::list_modify(pars,
                        model = "model2a_h3")

# Add in default priors
pars_model2a_h3 <- pars_model2a_h3 %>% default_priors()

# Add in parameters to data list
standata_model2a_h3 <- standata_size %>% add_pars(pars_model2a_h3)
...
print(model2a_h3_fit, probs=c(0.025, 0.5, 0.975))
```


```{r}
rstan::traceplot(model2a_h3_fit)
```

```{r, eval = FALSE}
shinystan::launch_shinystan(model2a_h3_fit)
```

Plot posteriors 

```{r}
posterior <- as.matrix(model2a_h3_fit)
...
par_1 <- sample_pars %>% 
  ggplot(aes(as.character(level3_index), 1+(Z_av/g_av), col = level3_index, group = level3_index)) +
  geom_boxplot() +
  geom_point() +
  coord_flip() +
  scale_y_log10(limits = c(0.9, 3.5), breaks = c(1, 2, 3)) +
