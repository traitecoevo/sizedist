...
model {
  ...
  for(i in 1:N_counts) {
    ID = sample_index[i];
        ...
        - R_t[ID]/Z_t[ID] * (exp(-Z_t[ID]/g_t[ID]*(bin_upper[i] - s0)) - exp(-Z_t[ID]/g_t[ID]*(bin_lower[i] - s0)));
  }
  ...
  for(i in 1:N_growth) {
    ...
    size_est[i] = s0 + age[i]*g_t[ind_sample_index[i]];
  }
