...
tmp <- gen_data %>%
      ...
      dplyr::mutate(
        ...
        individuals = list(
          ...
          dplyr::filter(pr_survival > runif(dplyr::n()))
        ),
...
#Can remove second day_born
all.equal(one_dayborn$individuals[[1]], orig_dayborn$individuals[[1]][,2:8])
all.equal(no_id$individuals[[1]], orig_dayborn$individuals[[1]][,-c(1,3)])
